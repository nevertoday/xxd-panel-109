...
# XXD Panel 109｜柔和な幾何解構誌

大きく完全な色面で、写真を一目で分かる幾何学の記憶へ解きほぐす
...
以下のサンプルは、異なる元画像から Panel 109 が個別に生成し、AI メタデータを除去したものです。

**16:9 横位置・左右サンプル**（左が元画像、右がデザイン、厳密な50:50）
...
**3:4 縦位置・上下サンプル**（上が元画像、下がデザイン、厳密な50:50）
...
写真には強い個性や感情がありますが、さらに意図的な再構成が必要なことがあります。**Panel 109** は写真の現実感を保ちながら、もう一方を独自の視覚言語へ翻訳します。アートポスター、出版物、展示、SNS、デザインのみの出力に適しています。
...
**キーワード：** 幾何学的解構 · 大きな色面モジュール · 柔らかく上質な配色 · 紙の粒子 · モダニズム・コラージュ · 編集的余白

## 4つの出力モード

- `top-bottom`：標準の3:4縦構図。現実の写真を上、Panel 109のデザインを下に置き、厳密に各50%とします。
...
識別しやすい写真でも情報が多すぎるとき、少数の大きく完全な幾何色面へ再構成します。矩形、角丸、楕円、三角、不規則面を使い、主体を細かな破片へ散らしません。

- 同一性を保ったモダニズム系ポスター、表紙、展示グラフィックに適します。
- 写真由来の2〜4色を柔らかく上質な配色へ整理し、紙粒子と編集的余白を加えます。
- 写実的なトレース、細かすぎる分割、幼いコラージュ、3D、テンプレート感を避けます。

## クイック適性チェック

| 確認点 | Panel 109 の答え |
|---|---|
| 完成物 | 現実写真と大色面の幾何学的再構成を組み合わせた完成ポスター |
| 特徴 | 少数の完全なモジュール、柔和な現代色、紙粒子、編集的余白 |
| 原写真の保持 | 同一性、構造、姿勢、関係を保ち、情報密度だけを変えます |

## 変換ロジックと境界

`同一性と構造を読む → 重要な輪郭と2〜4色を残す → 細かな背景を削る → 完全な幾何モジュールで再構成 → 正負形、紙粒子、余白を整える`
...
完全仕様：[SKILL.md](SKILL.md) · [実行アダプター](references/xxd-panel-109-prompt.en.md) · [原文プロンプト](references/original-prompt/zh-CN.md)
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜109 を連続して掲載し、現在のプロジェクトを太字で示します。
