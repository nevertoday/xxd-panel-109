...
# XXD Panel 109｜부드러운 기하 해체 기록

크고 완결된 색면으로 사진을 한눈에 알아보는 기하 기억으로 해체합니다
...
아래 샘플은 서로 다른 원본 참고 이미지에서 Panel 109가 독립적으로 생성했으며 AI 메타데이터를 제거했습니다.
...
사진은 이미 강한 정체성과 분위기를 담고 있지만, 더 의도적인 재구성이 필요할 때가 있습니다. **Panel 109**은 사진의 현실감을 유지하고 다른 절반을 독립적인 시각 언어로 번역합니다. 아트 포스터, 독립 출판, 전시 이미지, 소셜 콘텐츠와 디자인 전용 출력에 적합합니다.
...
**키워드:** 기하 해체 · 큰 색면 모듈 · 부드럽고 세련된 배색 · 종이 입자 · 모더니즘 콜라주 · 편집적 여백

## 네 가지 출력 모드

- `top-bottom`: 기본 3:4 세로 구조로, 현실 사진은 위에 Panel 109 디자인은 아래에 놓고 정확히 50%씩 사용합니다.
...
사진은 알아보기 쉽지만 정보가 너무 많을 때, 적고 크며 완결된 기하 색면으로 재구성합니다. 직사각형, 둥근 블록, 타원, 삼각형, 절제된 불규칙 면을 사용하며 주체를 자잘한 조각으로 흩뜨리지 않습니다.

- 정체성을 유지하는 모더니즘 포스터, 표지, 전시 그래픽에 적합합니다.
- 사진의 2–4색을 부드럽고 세련된 배색으로 정리하고 종이 입자와 편집 여백을 더합니다.
- 사실적 묘사, 과도한 조각화, 유아적 콜라주, 3D, 템플릿 효과를 피합니다.

## 빠른 판단과 변환 논리

`정체성과 구조 읽기 → 핵심 윤곽과 2–4색 유지 → 자잘한 배경 삭제 → 완결된 기하 모듈로 재구성 → 양형과 음형·종이 입자·여백 정리`
...
전체 사양: [SKILL.md](SKILL.md) · [실행 어댑터](references/xxd-panel-109-prompt.en.md) · [원본 프롬프트](references/original-prompt/zh-CN.md)
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–109을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
